...
server <- function(input, output) {
    ...
    data_performance <- reactive({
        ...
        data_mean <- filter(data, n == input$n, p == input$p, gamma == input$gamma)%>% 
            rename(`Huber`=`Huber-Mean`, `KRR` = `KRR-Mean`, `RKR`= `RKR-Mean`,
                   `KGARD`=`KGARD-Mean`) %>%
            select(`KRR`, `Huber`,`RKR`,`KGARD`,`Shift`) %>%
            pivot_longer(., cols = c(`KRR`, `Huber`,`RKR`,`KGARD`),
                         names_to = "method", values_to = "mean")
        #Aggregate data with sd
        data_sd <- filter(data, n == input$n, p == input$p, gamma == input$gamma) %>% 
    ...
    output$plot_performance<-renderPlotly({
        ...
        if(input$error_bars==FALSE){
            fig <- plot_ly(data_plot, x = ~`Shift`, y = ~`mean`, color = ~`method`,
                           mode = "lines+markers")
            ...
            fig <- plot_ly(data_plot, x = ~`Shift`, y = ~`mean`, color = ~`method`,
                           mode = "lines+markers", error_y=~list(array=data_plot$SD))
            fig <- fig %>% layout(yaxis = list(title="MSE"), xaxis = list(title="Outlier shift"),
                                  legend = list(x=0.05,y=1,title=list(text='<b> Methods </b>')),
                                  hovermode="compare")
            fig
    ...
    output$plot_complexity<-renderPlotly({
        ...
        #Plot results
        fig <- plot_ly(data = data, x = ~n, y = ~time, color = ~method, type="scatter",
                       mode="markers")
        if(input$complexity_fit==TRUE){
            fig <- fig %>% add_trace(data = data_fit, y =~ time_pred,type="scatter",
                                     mode ="lines", color=~method)
        }
        fig %>% layout(yaxis = list(title=y_title),
                       xaxis = list(title="n"),
                       legend = list(x=0.05,y=1,title=list(text='<b> Methods </b>')),
                       hovermode="compare")
    })
